...
export interface StaffId {
    id: string;
    name: string;
    className?: string; // Made optional for non-teaching staff
    dateAdded: Date;
    uid?: string;
    schoolId: string;
    isArchived: boolean;
...
export async function getStudents(db: Firestore, schoolId: string, includeArchived = false): Promise<Student[]> {
  ...
  const allStudents = studentSnapshot.docs
    ...
    .map(data => ({
        ...data,
        studentId: data.id, // studentId is the doc id
...
export async function getStudentById(db: Firestore, schoolId: string, studentId: string): Promise<Student | null> {
  ...
  const studentDocRef = doc(db, studentsCollection, studentId.toUpperCase());
  const studentSnap = await getDoc(studentDocRef).catch(serverError => {
    if (serverError.code === 'permission-denied') {
        const permissionError = new FirestorePermissionError({
...
export async function addStudent(db: Firestore, auth: Auth, schoolId: string, newStudentData: Omit<Student, 'dateAdded' | 'profilePicture' | 'generalFees' | 'generalPayments' | 'dailyFeedingCost' | 'feedingFeePayments' | 'attendance' | 'transportationCost' | 'transportationPayments' | 'isArchived'>) {
    ...
    const studentDocRef = doc(db, studentsCollection, upperCaseStudentId);
...
export async function archiveStudent(db: Firestore, auth: Auth, studentId: string, archive = true) {
  await ensureUserAuthenticated(auth); // Ensure user is authenticated
  const studentDocRef = doc(db, studentsCollection, studentId);
...
export async function deleteStudent(db: Firestore, storage: FirebaseStorage, auth: Auth, studentId: string) {
...
export async function updateStudentId(db: Firestore, auth: Auth, oldStudentId: string, newStudentId: string) {
    ...
    const newIdDocRef = doc(db, studentsCollection, upperCaseNewStudentId);
    const oldDocRef = doc(db, studentsCollection, oldStudentId);
...
export async function updateStudentDetails(db: Firestore, storage: FirebaseStorage, auth: Auth, studentId: string, details: Partial<Omit<Student, 'studentId' | 'dateAdded'>>, photoFile: File | null) {
...
export async function updateGeneralFees(db: Firestore, auth: Auth, studentId: string, fees: FeeItem[]) {
  await ensureUserAuthenticated(auth);
  const studentDocRef = doc(db, studentsCollection, studentId);
...
export async function addGeneralPayment(db: Firestore, auth: Auth, studentId: string, payment: Omit<PaymentItem, 'id'>) {
  await ensureUserAuthenticated(auth);
  const studentDocRef = doc(db, studentsCollection, studentId);
  const studentSnap = await getDoc(studentDocRef);
...
export async function deleteGeneralPayment(db: Firestore, auth: Auth, studentId: string, paymentId: number) {
  await ensureUserAuthenticated(auth);
  const studentDocRef = doc(db, studentsCollection, studentId);
  const studentSnap = await getDoc(studentDocRef);
  const student = studentSnap.data() as Student | undefined;
...
export async function updateDailyCost(db: Firestore, auth: Auth, studentId: string, cost: number) {
  await ensureUserAuthenticated(auth);
  const studentDocRef = doc(db, studentsCollection, studentId);
...
export async function addFeedingPayment(db: Firestore, auth: Auth, studentId: string, payment: Omit<PaymentItem, 'id'>) {
    await ensureUserAuthenticated(auth);
    const studentDocRef = doc(db, studentsCollection, studentId);
    const studentSnap = await getDoc(studentDocRef);
    const student = studentSnap.data() as Student | undefined;
...
export async function deleteFeedingPayment(db: Firestore, auth: Auth, studentId: string, paymentId: number) {
  await ensureUserAuthenticated(auth);
  const studentDocRef = doc(db, studentsCollection, studentId);
  const studentSnap = await getDoc(studentDocRef);
  const student = studentSnap.data() as Student | undefined;
...
export async function updateTransportationCost(db: Firestore, auth: Auth, studentId: string, cost: number) {
  await ensureUserAuthenticated(auth);
  const studentDocRef = doc(db, studentsCollection, studentId);
...
export async function addTransportationPayment(db: Firestore, auth: Auth, studentId: string, payment: Omit<PaymentItem, 'id'>) {
    await ensureUserAuthenticated(auth);
    const studentDocRef = doc(db, studentsCollection, studentId);
    const studentSnap = await getDoc(studentDocRef);
    const student = studentSnap.data() as Student | undefined;
...
export async function deleteTransportationPayment(db: Firestore, auth: Auth, studentId: string, paymentId: number) {
  await ensureUserAuthenticated(auth);
  const studentDocRef = doc(db, studentsCollection, studentId);
  const studentSnap = await getDoc(studentDocRef);
  const student = studentSnap.data() as Student | undefined;
...
export async function setAttendance(db: Firestore, auth: Auth, studentId: string, date: string, attended: boolean) {
    const studentDocRef = doc(db, studentsCollection, studentId);
    const studentSnap = await getDoc(studentDocRef);
    const student = studentSnap.data() as Student | undefined;
    if (student) {
        ...
        if (existingRecordIndex > -1) {
            ...
            newAttendance.push({ id: Date.now(), date, attended });
        }
        
        newAttendance.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
        await updateDoc(studentDocRef, { attendance: newAttendance }).catch(async (serverError) => {
            const permissionError = new FirestorePermissionError({
                path: studentDocRef.path,
                operation: 'update',
                requestResourceData: { attendance: newAttendance },
            });
            errorEmitter.emit('permission-error', permissionError);
            throw permissionError;
        });
...
// --- ANNOUNCEMENTS ---
export async function sendAnnouncement(db: Firestore, auth: Auth, schoolId: string, announcement: Omit<Announcement, 'id' | 'date' | 'schoolId'>) {
    if (!schoolId) throw new Error("School ID missing.");
    await ensureUserAuthenticated(auth);
...
export async function addStaffId(db: Firestore, auth: Auth, schoolId: string, name: string, staffId?: string, className?: string) {
    if (!schoolId) throw new Error("School ID missing.");
    await ensureUserAuthenticated(auth);
...
// --- EXPENDITURE ---
export async function getExpenditures(db: Firestore, schoolId: string): Promise<Expenditure[]> {
    if (!schoolId) return [];
    const expenditureCol = collection(db, expenditureCollection);
    const q = query(expenditureCol, where('schoolId', '==', schoolId.toUpperCase()));
    const snapshot = await getDocs(q).catch(serverError => {
        if (serverError.code === 'permission-denied') {
            const permissionError = new FirestorePermissionError({
...
// --- DEBTS & LIABILITIES ---
export async function getDebts(db: Firestore, schoolId: string): Promise<Debt[]> {
    if (!schoolId) return [];
    const debtsCol = collection(db, debtsCollection);
    const q = query(debtsCol, where('schoolId', '==', schoolId.toUpperCase()));
    const snapshot = await getDocs(q).catch(serverError => {
        if (serverError.code === 'permission-denied') {
            const permissionError = new FirestorePermissionError({
                path: debtsCol.path,
